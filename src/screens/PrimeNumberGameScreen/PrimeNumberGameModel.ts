...
export class PrimeNumberGameModel {
    ...
    public submitAnswer(userInput: string): { isCorrect: boolean, correctAnswerText: string, newScore: number, isGameOver: boolean } {
        const question = this.getCurrentQuestion();
        if (!question) return { isCorrect: false, correctAnswerText: 'N/A', newScore: this.score, isGameOver: true };
...
class MarsQuestionGenerator {
    public getQuestions(maxNumberOfQuestions: number): Promise<MarsQuestion[]> {
        ...
        for (let i = 0; i < maxNumberOfQuestions; i++) {
            ...
            bank.push(new MarsQuestion(
                `Is ${numToTest} a prime number?`,
                numToTest,
                answer,
                10
            ));
