...
export class Player extends GameObject {
    ...
    attachNode(node: Konva.Node): void {
        ...
        try {
            ...
            this.halfHeight = h / 2;
            if (this.collidable) {
                this.collidable.setRect(this.model.x - this.halfWidth, this.model.y - this.halfHeight, w, h);
            }
        } catch {
            // ignore
        }
    ...
    update(deltaTimeMs: number): void {
        ...
        if (this.collidable && this.collidable.shape && (this.collidable.shape as any).w !== undefined) {
