...
import {
  Theme, getDefaultTheme, weightToFontStyle, lighten,
  CORNER_RADIUS, STROKE_WIDTH, BUTTON_LISTENING, TEXTBOX_LISTENING, getDefaultX
} from "../theme/Theme";
...
export function createButton(args: ButtonArgs, theme?: Theme): Konva.Group {
  ...
  const hoverFill = args.hoverColorKey && t.has(args.hoverColorKey) ? t.get(args.hoverColorKey) : lighten(fill);
...
export function setElementText(group: Konva.Group, newText: string) {
  ...
  if (group.listening()) {
    ...
    if (!fit) console.warn(`[UI] Button text overflow after update: "${newText}" (w=${rect.width()}px).`);
  } else {
...
export function resizeElement(group: Konva.Group, width: number, height: number) {
  ...
  if (group.listening()) {
    ...
    group.setAttr("_overflow", !fit);
    if (!fit) console.warn(`[UI] Button text overflow after resize (w=${width}px, text="${txt.text()}").`);
  } else {
    // textbox
    const pad = (txt.x?.() ?? 12);
