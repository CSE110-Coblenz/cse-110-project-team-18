...
export function setStageWidth(w: number) { STAGE_WIDTH = w; }
export function getDefaultX(): number { return STAGE_WIDTH / 2 - 200; }
...
export class Theme {
  ...
  has(key: string) { return this._colors.has(key); }
}

let __defaultTheme: Theme | null = null;
export function setDefaultTheme(t: Theme) { __defaultTheme = t; }
...
export function lighten(hex: string, amount = 0.10): string {
  const h = hex.replace("#", "");
